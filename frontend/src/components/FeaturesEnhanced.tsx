...
import { useFadeInUp, useScaleIn } from '@/hooks/useGSAP';

const FeaturesEnhanced = () => {
  const titleRef = useFadeInUp(0);
  const descRef = useFadeInUp(0.2);
    ...
    <section id="features" className="py-20 px-6">
      <div className="container mx-auto">
        <div className="text-center mb-16">
          ...
          <p ref={descRef} className="text-xl text-muted-foreground max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            const featureRef = useScaleIn(index * 0.1);
            
            return (
              <div 
                key={feature.title}
                ref={featureRef}
                className="glass dark:glass-dark p-8 rounded-3xl hover:scale-105 transition-all duration-300 cursor-pointer"
              >
